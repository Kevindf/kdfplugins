...
sub setMode {
	...
	@browseMenuChoices = (
		...
		$client->string('PLUGIN_ALARM_DAYS'),
		);
...
sub initPlugin {
	...
	%functions = (
		...
		'right' => sub  {
			...
			if ($browseMenuChoices[$menuSelection{$client}] eq $client->string('ALARM_SET')) {
			...
			elsif ($browseMenuChoices[$menuSelection{$client}] eq $client->string('ALARM_SELECT_PLAYLIST')) {
				my @dirItems=();	
				...
				if ((grep {$_ eq 'Random'} keys %{Slim::Buttons::Plugins::installedPlugins()}) 
					&& !(grep {$_ eq 'Random'} Slim::Utils::Prefs::getArray('disabledplugins'))) {
				...
				my %params = (
					...
					,'overlayRef' => sub {
							if (defined $_[1] && $_[1] ne $client->string('PLUGIN_ALARM_CURRENT_PLAYLIST') && Slim::Music::Info::isDir($_[1])) {
								my @overlay;
								push @overlay,' ';
								push @overlay,Slim::Display::Display::symbol('rightarrow');
								return @overlay;
							} else {return undef;};
						}
			...
			elsif ($browseMenuChoices[$menuSelection{$client}] eq $client->string('PLUGIN_ALARM_DAYS')) {
				my %params = (
		...
		'play' => sub {
			...
			if ($browseMenuChoices[$menuSelection{$client}] eq $client->string('ALARM_SELECT_PLAYLIST') 
				...
				$client->showBriefly($client->string('PLUGIN_ALARM_SETTING_PLAYLIST'),
					(($value eq $client->string("PLUGIN_ALARM_CURRENT_PLAYLIST")) 
					|| ($value eq $client->string("PLUGIN_RANDOM"))) 
						? $value 
...
sub lines {
	...
	if (Slim::Utils::Prefs::clientGet($client, 'PLUGIN.alarm.'.$alarmID) && $browseMenuChoices[$menuSelection{$client}] eq $client->string('ALARM_OFF')) {
	$browseMenuChoices[$menuSelection{$client}] = $client->string('ALARM_ON');
	}
	$line2 = "";
...
#		if ($browseMenuChoices[$menuSelection{$client}] eq $client->string('ALARM_SELECT_PLAYLIST') 
#				&& Slim::Music::Info::isDir(${$client->param('valueRef')})) {
#			my @dirItems=();
#			Slim::Utils::Scan::addToList(\@dirItems, Slim::Utils::Misc::pathFromFileURL(${$client->param('valueRef')}), 0);
#			my $alarm = Slim::Utils::Prefs::clientGet($client,'PLUGIN.alarmplaylist.'.$alarmID) || \$dirItems[0];
#			my %params = (
#				'listRef' => \@dirItems
...
#				,'overlayRef' => sub {
#						if (defined $_[1] 
#								&& $_[1] ne $client->string('PLUGIN_ALARM_CURRENT_PLAYLIST') 
#								&& $_[1] ne $client->string('PLUGIN_RANDOM')
#								&& Slim::Music::Info::isDir($_[1])) {
#							my @overlay;
#							push @overlay,' ';
#							push @overlay,Slim::Display::Display::symbol('rightarrow');
#							return @overlay;
...
sub alarmActiveLines {
	...
	if ($snooze) {
		...
		$parts->{line1} .= "(".int(($countdown - $time + 50)/60)." ".$client->string('MINUTES').")";
	} elsif ($playlistlen < 1) {
...
sub snooze {
	...
	if (!$snooze) {
		...
		$countdown = Time::HiRes::time() + ($snoozetime * 60);
		$::d_plugins && msg("Alarm Plugin $alarmID: Coundown: $countdown\n");
		Slim::Utils::Timers::setTimer($client, $countdown, \&unSnooze, $client);
		$client->showBriefly($name || $client->string(getDisplayName()), $client->string('PLUGIN_ALARM_SNOOZE'));
	}
};
...
sub killAlarm {
	...
	$client->showBriefly($name || $client->string(getDisplayName()), $client->string('ALARM_TURNING_OFF'),3);
}
...
sub unSnooze {
	...
	$client->showBriefly($client->string('PLUGIN_ALARM_NOW_PLAYING'), $client->string('PLUGIN_ALARM_WAKEUP'));
...
sub alarmTrigger {
	...
	if ((defined ($volume)) && !$snooze) {
		Slim::Control::Command::execute($client, ["mixer", "volume", $volume]);
	}
	if ((defined Slim::Utils::Prefs::clientGet($client, 'PLUGIN.alarmplaylist.'.$alarmID)) && !$snooze) {
